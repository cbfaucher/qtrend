...
public class FindIncomingOssAction extends FindIncomingSignalAction
{
    ...
    public void handleEvent(QEventManager pEventManager, EventObject pEvent, String pCommand)
    {
        ...
        try
        {
            ...
            final String rsiStr = JOptionPane.showInputDialog(frame, "RSI above?", userProperties.getPropertyAsInt(QTrendConstants.UserPropertyNames.USERPROP_OSS_MINIMUM_RSI, 70));
            if (rsiStr == null) return;
            final int minRsi = Integer.parseInt(rsiStr.trim());
            userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_OSS_MINIMUM_RSI, String.valueOf(minRsi));
