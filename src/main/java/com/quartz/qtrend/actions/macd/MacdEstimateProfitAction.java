...
public class MacdEstimateProfitAction extends AbstractEstimateProfitAction
{
    ...
    protected boolean askQuestions()
    {
        ...
        final String obsRsiStr = JOptionPane.showInputDialog(frame,
                                                             "OBS RSI below?",
                                                             userProperties.getPropertyAsInt(QTrendConstants.UserPropertyNames.USERPROP_OBS_MAX_RSI, 30));
        if (obsRsiStr == null) return false;
        obsRsi = Integer.parseInt(obsRsiStr.trim());
        userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_OBS_MAX_RSI, String.valueOf(obsRsi));

        final String ossRsiStr = JOptionPane.showInputDialog(frame,
                                                             "OSS RSI above?",
                                                             userProperties.getPropertyAsInt(QTrendConstants.UserPropertyNames.USERPROP_OSS_MINIMUM_RSI, 30));
        if (ossRsiStr == null) return false;
        ossRsi = Integer.parseInt(ossRsiStr.trim());
        userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_OSS_MINIMUM_RSI, String.valueOf(ossRsi));
