...
public class FindMacdObsAction extends AbstractFindObsAction
{
    ...
    protected boolean askQuestions()
    {
        ...
        final String rsiStr = JOptionPane.showInputDialog(parent, "RSI below?", userProperties.getPropertyAsInt(QTrendConstants.UserPropertyNames.USERPROP_OBS_MAX_RSI, 30));
        if (rsiStr == null) return false;
        maximumRsi = Integer.parseInt(rsiStr.trim());
        userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_OBS_MAX_RSI, String.valueOf(maximumRsi));
