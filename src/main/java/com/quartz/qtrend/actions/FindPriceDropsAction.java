...
//            if (exchangeName == null) return;
            ...
            final LocalDate fromDate = ActionHelper.askDate(parent, "Since Date?", new LocalDate().toString());
...
//                                                            (dates.length > 0 ? dates[0] : null));
            ...
            final String minPriceDropStr = JOptionPane.showInputDialog(
                    parent, "Drop for at least x %?", userProperties.getPropertyAsFloat(QTrendConstants.UserPropertyNames.USERPROP_LAST_MINIMUM_DROP, 10.0f));
            if (minPriceDropStr == null) return;
            final float minPriceDrop = new Float(minPriceDropStr.trim()).floatValue();
            userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_LAST_MINIMUM_DROP, String.valueOf(minPriceDrop));
