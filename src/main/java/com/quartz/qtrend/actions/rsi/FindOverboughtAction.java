...
public class FindOverboughtAction implements QEventHandler, JFrameAware<QTrendFrame>, QTrendConstants.UserPropertyNames
{
    ...
    public void handleEvent(QEventManager pEventManager, EventObject pEvent, String pCommand)
    {
        ...
        final String minimumPriceStr = JOptionPane.showInputDialog(frame, "Minimum price?", userProperties.getPropertyAsFloat(USERPROP_MINIMUM_PRICE, 1.00f));
        if (minimumPriceStr == null) return;
        final float minimumPrice = new Float(minimumPriceStr).floatValue();
        userProperties.setProperty(USERPROP_MINIMUM_PRICE, String.valueOf(minimumPrice));

        final String volumeStr = JOptionPane.showInputDialog(frame, "Minimum volume?", userProperties.getPropertyAsInt(USERPROP_MINIMUM_VOLUME, 100000));
        if (volumeStr == null) return;
        final int minimumVolume = Integer.parseInt(volumeStr.trim());
        userProperties.setProperty(USERPROP_MINIMUM_VOLUME, String.valueOf(minimumVolume));

        final String rsiStr = JOptionPane.showInputDialog(frame, "RSI above?", userProperties.getPropertyAsInt(USERPROP_OSS_MINIMUM_RSI, 70));
        if (rsiStr == null) return;
        final int minRsi = Integer.parseInt(rsiStr.trim());
        userProperties.setProperty(USERPROP_OSS_MINIMUM_RSI, String.valueOf(minRsi));
