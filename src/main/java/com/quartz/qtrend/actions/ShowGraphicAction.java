...
public class ShowGraphicAction implements QEventHandler, JFrameAware<QTrendFrame>
{
    ...
    public void handleEvent(QEventManager pEventManager, EventObject pEvent, String pCommand)
    {
        ...
        {
            selectedText = JOptionPane.showInputDialog(parent, "What is the ticker?", userProperties.getProperty(
                    QTrendConstants.UserPropertyNames.USERPROP_LAST_TICKER, ""));
            if (selectedText == null) return;
            userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_LAST_TICKER, selectedText);
        }

        final Ticker ticker = new Ticker(selectedText);

        final String periodStr = JOptionPane.showInputDialog(parent, "For how many periods in the past (~20 per month)?", userProperties.getPropertyAsInt(
        ...
        try
        {
            ...
            userProperties.setProperty(USERPROP_PERIODS_BACK, periodStr);
