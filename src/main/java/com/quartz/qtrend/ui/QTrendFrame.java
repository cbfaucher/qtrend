...
public class QTrendFrame extends SpringFrame implements QEventFrame
{
    ...
    private static final String USERPROP_DIVIDER_LOCATION = "com.quartz.qtrend.divider.location";
...
//        if (pMessageHandler instanceof Component == false) throw new IllegalArgumentException("QTrend expects the message handler to be a java.awt.Component.");
    ...
    public void setVisible(boolean b)
    {
        if (b == true)
        {
            setX(userProperties.getPropertyAsInt(QTrendConstants.UserPropertyNames.USERPROP_X, 50));
            setY(userProperties.getPropertyAsInt(QTrendConstants.UserPropertyNames.USERPROP_Y, 50));

            setWidth(userProperties.getPropertyAsInt(QTrendConstants.UserPropertyNames.USERPROP_WIDTH, 500));
            setHeight(userProperties.getPropertyAsInt(QTrendConstants.UserPropertyNames.USERPROP_HEIGHT, 300));

            super.setVisible(true);

            splitPane.setDividerLocation(userProperties.getPropertyAsFloat(USERPROP_DIVIDER_LOCATION, 0.80f));
...
//            log4jAppender.attach();
        ...
        else
        {
            ...
            userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_X, "" + getX());
            userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_Y, "" + getY());

            userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_WIDTH, "" + getWidth());
            userProperties.setProperty(QTrendConstants.UserPropertyNames.USERPROP_HEIGHT, "" + getHeight());

            userProperties.setProperty(USERPROP_DIVIDER_LOCATION, "" + ( ((float) splitPane.getDividerLocation()) / getSize().height));
