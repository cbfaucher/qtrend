...
public class ImportYahooTickerHistoryFromWeb implements QEventHandler, JFrameAware<QTrendFrame>
{
    ...
    public void handleEvent(QEventManager pEventManager, EventObject pEvent, String pCommand)
    {
        ...
        final String lastStartDate = userProperties.getUserProperty(USERPROP_LAST_START_DATE, "2006/06/01");
        final YearMonthDay startDate = ActionHelper.askDate(frame, "From Date", lastStartDate);
        if (startDate == null) return;
        userProperties.setUserProperty(USERPROP_LAST_START_DATE, startDate.toString());

        final YearMonthDay endDate = ActionHelper.askDate(frame, "To Date:", new YearMonthDay().toString());
